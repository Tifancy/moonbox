...
trait PersistInterface {
	...
	def addWaitingJob(job: JobState): Boolean = {
		add[JobState](WAITING_DIR, job.jobId, job)
	}
	...
	def addRunningJob(job: JobState): Boolean = {
		add[JobState](RUNNING_DIR, job.jobId, job)
	}
	...
	def addCompleteJob(job: JobState): Boolean = {
		add[JobState](COMPLETE_DIR, job.jobId, job)
	}
	...
	def addFailedJob(job: JobState): Boolean = {
		add[JobState](FAILED_DIR, job.jobId, job)
	}
	...
	def addWorkerJob(worker: ActorRef, job: String): Boolean = {
		...
		add[ActorRef](WORKER_TO_RUNNING_JOBS, workerKey, worker)
		add[String](s"$WORKER_TO_RUNNING_JOBS/$workerKey", KEY_UNDER_WORKER, job)
	}
	...
	def setCacheURL(url: String): Boolean = {
		add[String](CACHE_DIR, CACHE_KEY, url)
	}
	...
	def setMasterURL(url: String): Boolean = {
		add[String](MASTERS_DIR, MASTER_KEY, url)
	}

	def add[T: ClassTag](path: String, key: String, data: T): Boolean
...
class ZkPersist(conf: MbConf)(implicit val system: ActorSystem)
	...
	override def add[T: ClassTag](path: String, key: String, job: T): Boolean = {
